...
def izip(*cubes, **kwargs):
    ...
    * coords (string, coord or a list of strings/coords): 
        Coordinate names/coordinates of the desired sub cubes (i.e. the coords that are not iterated 
        over). They must all be orthogonal (i.e. point to different dimensions).
    * ordered (Boolean):
        If True (default), the order which the coords to slice are given will be the order in which
        they represent the data in the resulting slices
    ...
    # Convert any coordinate names to coordinates (and ensure each cube has requested slice coords)
    coords_to_slice = kwargs.get('coords')
    coords_by_cube = []

    for cube in cubes:
        if coords_to_slice == None or not coords_to_slice:
            coords_by_cube.append([])
        else:    
            coords_by_cube.append(cube._as_list_of_coords(coords_to_slice))

    # For each input cube, generate the union of all describing dimensions for the resulting subcube 
    ...
    for coords, cube in zip(coords_by_cube, cubes):
        ...
        # Make sure this cube has no shared dimensions between the requested coords.
        if len(requested_dims) != sum((len(cube.coord_dims(coord)) for coord in coords)):
            raise ValueError('The requested coordinates (%r) of cube (%r) are not orthogonal.' 
                             % ([coord.name() for coord in coords], cube))
    ...
    # dimensioned coordinates that will be iterated over (i.e exclude slice coords)
    dimensioned_iter_coords_by_cube = []
    for requested_dims, cube in zip(requested_dims_by_cube, cubes):
        dimensioned_iter_coords = set()
        for dim in range(len(cube.shape)):        # loop over dimensioned coords in each cube
    ...
    # Check for multidimensional coords - current implementation cannot iterate over multidimensional coords
    #for dimensioned_iter_coords in dimensioned_iter_coords_by_cube:
    #    for coord in dimensioned_iter_coords:
    #        if coord.ndim > 1:
    #            raise iris.exceptions.CoordinateMultiDimError(coord)

    # Iterate through all the possible pairs of cubes to compare dimensioned coordinates
    for dimensioned_iter_coords_a, dimensioned_iter_coords_b in itertools.combinations(dimensioned_iter_coords_by_cube, 2):
        coords_by_def_a = set((_CoordWrapper(coord) for coord in dimensioned_iter_coords_a))
        coords_by_def_b = set((_CoordWrapper(coord) for coord in dimensioned_iter_coords_b))

        # Check to see if one cube is not a 'subspace' of the other, i.e. raise exception if cube_a has dimensioned
        # coords that cube_b doesn't have, and cube_b has dimensioned coords that cube_a doesn't have. 
        # _ZipSlicesIterator will handle the case where this is true and will iterate through both separately, 
        # but it is sufficiently unlikely that the user really intends to do this that we catch it and raise an exception.
        unique_a = coords_by_def_a - coords_by_def_b
        unique_b = coords_by_def_b - coords_by_def_a
        if len(unique_a) != 0 and len(unique_b) != 0:
            raise ValueError("More than one cube contains a unique dimensioned coordinate")
        
        # Check that the dimensioned coords that are common across the cubes (i.e. have same definition/metadata)
        # have the same shape. If this is not the case it makes no sense to iterate through the coordinate in step
        # and an exception is raised.
        common = coords_by_def_a & coords_by_def_b
        for definition_coord in common:
            # Extract matching coord from dimensioned_iter_coords_a and dimensioned_iter_coords_b to access shape
            coord_a = next((coord for coord in dimensioned_iter_coords_a if definition_coord == coord))
            coord_b = next((coord for coord in dimensioned_iter_coords_b if definition_coord == coord))
...
class _ZipSlicesIterator(collections.Iterator):
    """
    Extension to _SlicesIterator (see cube.py) to support iteration over a collection of cubes in step
    ...
    def __init__(self, cubes, requested_dims_by_cube, ordered, coords_by_cube):
        ...
        # Check that the requested_dims_by_cube and coords_by_cube lists are the same length as cubes
        # so it is feasible that there is a 1-1 mapping of values (itertool.izip won't catch this)
        if len(requested_dims_by_cube) != len(cubes):
            raise ValueError('requested_dims_by_cube parameter is not the same length as cubes.')
        ...
        for requested_dims, cube in zip(requested_dims_by_cube, cubes):
            # Create a list of the shape of each cube, and set the dimensions which have been requested to length 1
            dims_index = list(cube.shape)
            for dim in requested_dims:
                dims_index[dim] = 1
            offsets = []
            for i in range(len(cube.shape)):           # loop over dimensions in each cube
                cube_coords = cube.coords(dimensions=i)    # obtain the coordinates for this dimension
                found = False
                for coord in cube_coords:               # loop over coords in this dimension (could be just one)
                    # Search for coord in master_dimensioned_coord_list
                    for j, master_coords in enumerate(master_dimensioned_coord_list):
                        if _CoordWrapper(coord) in master_coords:       # use coord wrapper with desired equality functionality
                            offsets.append(j)
                            found = True
                            break
                    if found:
                        break
                # If a coordinate with an equivalent definition (i.e. same metadata) is not
                # found in the master_dimensioned_coord_list, add the coords assocaited with
                # the dimension to the list, add the size of the dimension to the master_dims_index
                # and store the offset
                if not found:
                    master_dimensioned_coord_list.append(set((_CoordWrapper(coord) for coord in cube_coords)))
                    master_dims_index.append(dims_index[i])
                    offsets.append(len(master_dims_index)-1)
            # Store the offsets for each cube so they can be used in _ZipSlicesIterator.next()
    ...
    def __next__(self):
        ...
        subcubes = []
        for offsets, requested_dims, coords, cube in zip(self._offsets_by_cube, 
            ...
            # Call transpose if necessary (taken from _SlicesIterator in cube.py)
...
class _CoordWrapper:
    """
    Class for creating a coordinate wrapper that allows the use of an alternative equality function based 
    on metadata rather than metadata + points/bounds.

    NB. Uses a lightweight/incomplete implementation of the Decorator pattern.
