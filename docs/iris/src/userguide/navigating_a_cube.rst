...
Cubes can also contain additional arbitrary metadata stored in either the :attr:`~iris.cube.Cube.attributes`
...
can be accessed and modified in the usual way, for example::
    ...
    print cube.attributes['source']
    del cube.attributes['source']
    cube.attributes.update(other_attributes)
